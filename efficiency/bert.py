...
from utils import softmax_2RELU, softmax_2QUAD, secformer_softmax, puma_softmax
from utils import activation_quad, crypten_gelu, secformer_gelu, puma_gelu, gelu_erf
from utils import crypten_norm, puma_norm, secformer_norm
...
################################layernorm########################################################
        if config.norm == "crypten_norm":
            ...
            self.LayerNorm = puma_norm(config.hidden_size, eps=config.layer_norm_eps)
        elif config.norm == "secformer_norm":
            self.LayerNorm = secformer_norm(config.hidden_size, eps=config.layer_norm_eps)
        else:
            raise ValueError(f"activation type {config.norm} not implemented")
...
class BertSelfAttention(cnn.Module):
    def __init__(self, config, timing):
        ...
        elif config.softmax_act == "2quad":
            self.norm = cnn.BatchNorm2d(config.hidden_size, eps=config.layer_norm_eps)
            self.smax = softmax_2QUAD(self.norm, dim=-1)
        elif config.softmax_act == 'secformer_softmax':
            self.norm = cnn.BatchNorm2d(config.hidden_size, eps=config.layer_norm_eps)
            self.smax = secformer_softmax(self.norm, dim=-1)
        elif config.softmax_act == 'puma_softmax':
            self.norm = cnn.BatchNorm2d(config.hidden_size, eps=config.layer_norm_eps)
            self.smax = puma_softmax(self.norm, dim=-1)
        else:
...
class BertIntermediate(cnn.Module):
    def __init__(self, config, timing):
        ...
        elif config.hidden_act == "gelu_erf":
            self.intermediate_act_fn = gelu_erf()
        elif config.hidden_act == "crypten_gelu":
            self.intermediate_act_fn = crypten_gelu()
        elif config.hidden_act == "puma_gelu":
            self.intermediate_act_fn = puma_gelu()
        elif config.hidden_act == "secformer_gelu":
            self.intermediate_act_fn = secformer_gelu((1,512,3072))
        else:
